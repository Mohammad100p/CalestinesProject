...
namespace FS_CombatCore
{
    ...
    public class CombatAIController : MonoBehaviour
    {
        ...
        public IParkourAICharacter iParkourAICharacter { get; private set; }
        ...
        private void Start()
        {
            ...
            iParkourAICharacter = GetComponent<IParkourAICharacter>();
            ...
            Fighter.OnBeingAttacked += (FighterCore attacker) =>
            {
                ...
                if (Fighter.CanDodge && Fighter.Action != FighterAction.Dodging && !IsInState(AIStates.Attack) && (canInteruptActionToDodge || !Fighter.IsBusy))
                {
            ...
            Fighter.OnGotHit += (FighterCore attacker, Vector3 hitPoint, float hittingTime, bool isBlockedHit, HitType HitType) =>
            {
                if (Fighter.Damagable.CurrentHealth > 0)
                {
                    ...
                    if (!isBlockedHit && aiCombatType == AICombatType.Melee)
